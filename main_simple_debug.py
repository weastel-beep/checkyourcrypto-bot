    ...
    try:
        ...
        if callback.data.startswith("free_check:"):
            ...
            try:
                ...
                logger.info(f"🔍 ВЫЗЫВАЕМ wallet_screening для адреса {address} и сети {chain}")
                screening_result = await wallet_screening(address, chain)
                logger.info(f"✅ ПОЛУЧЕН результат wallet_screening: {screening_result}")
                
                logger.info(f"🔍 ВЫЗЫВАЕМ address_label для адреса {address} и сети {chain}")
                label_result = await address_label(address, chain)
                logger.info(f"✅ ПОЛУЧЕН результат address_label: {label_result}")
...
💬 **Выберите тип проверки**"""
                        ...
                        try:
                            ...
                            logger.info(f"🔍 ВЫЗЫВАЕМ wallet_screening для адреса {message.text} и сети {chain}")
                            screening_result = await wallet_screening(message.text, chain)
                            logger.info(f"✅ ПОЛУЧЕН результат wallet_screening: {screening_result}")
                            
                            logger.info(f"🔍 ВЫЗЫВАЕМ address_label для адреса {message.text} и сети {chain}")
