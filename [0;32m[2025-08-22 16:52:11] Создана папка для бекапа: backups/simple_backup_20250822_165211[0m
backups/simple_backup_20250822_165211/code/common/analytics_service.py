    ...
    async def get_user_stats(user_id: int) -> Dict[str, Any]:
        ...
        async with async_session_maker() as session:
            ...
            total_checks = await session.execute(
                "SELECT COUNT(*) FROM check_history WHERE user_id = :user_id",
                {"user_id": user_id}
            )
            
            paid_checks = await session.execute(
                "SELECT COUNT(*) FROM check_history WHERE user_id = :user_id AND check_type = 'paid'",
                {"user_id": user_id}
            )
            
            free_checks = await session.execute(
                "SELECT COUNT(*) FROM check_history WHERE user_id = :user_id AND check_type = 'free'",
                {"user_id": user_id}
            )
            ...
            total_spent = await session.execute(
                "SELECT SUM(amount) FROM payments WHERE user_id = :user_id AND status = 'completed'",
                {"user_id": user_id}
            )
    ...
    async def get_global_stats() -> Dict[str, Any]:
        ...
        async with async_session_maker() as session:
            ...
            active_users = await session.execute(
                "SELECT COUNT(*) FROM users WHERE last_activity >= :date",
            ...
            paid_checks = await session.execute(
                "SELECT COUNT(*) FROM check_history WHERE check_type = 'paid'"
            )
            ...
            recent_checks = await session.execute(
                "SELECT COUNT(*) FROM check_history WHERE created_at >= :date",
                {"date": thirty_days_ago}
            )
