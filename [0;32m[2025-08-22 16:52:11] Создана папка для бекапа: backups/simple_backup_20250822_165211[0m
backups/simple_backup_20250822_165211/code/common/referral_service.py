...
class ReferralService:
    ...
    async def get_user_referral_code(user_id: int) -> Optional[str]:
        ...
        async with async_session_maker() as session:
            code = await session.execute(
                "SELECT code FROM referral_codes WHERE user_id = :user_id AND is_active = true",
                {"user_id": user_id}
            )
    ...
    async def get_user_by_referral_code(code: str) -> Optional[int]:
        ...
        async with async_session_maker() as session:
            user = await session.execute(
                "SELECT user_id FROM referral_codes WHERE code = :code AND is_active = true",
                {"code": code}
            )
    ...
    async def process_referral(referrer_id: int, referred_id: int) -> bool:
        ...
        async with async_session_maker() as session:
            ...
            existing = await session.execute(
                "SELECT id FROM users WHERE tg_id = :referred_id AND referred_by IS NOT NULL",
    ...
    async def process_payment_reward(referred_id: int, payment_amount: Decimal) -> bool:
        ...
        async with async_session_maker() as session:
            ...
            referrer = await session.execute(
                "SELECT referred_by FROM users WHERE tg_id = :referred_id",
                {"referred_id": referred_id}
            )
