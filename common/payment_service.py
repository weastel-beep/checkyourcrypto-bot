...
class PaymentService:
    ...
    async def get_payment_stats() -> Dict[str, Any]:
        ...
        async with async_session_maker() as session:
            # Общая статистика
            total_payments = await session.execute("SELECT COUNT(*) FROM payments WHERE status = 'completed'")
            total_amount = await session.execute("SELECT SUM(amount) FROM payments WHERE status = 'completed'")

            # Статистика за последние 30 дней
            thirty_days_ago = datetime.utcnow() - timedelta(days=30)
            recent_payments = await session.execute(
                "SELECT COUNT(*) FROM payments WHERE status = 'completed' AND created_at >= :date", {"date": thirty_days_ago}
            )
