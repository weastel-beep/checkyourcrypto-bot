...
from common.models import User, MassMessage, MassMessageRecipient, MassMessageStatus, OutboxStatus, Outbox
...
class MassMessagingService:
    ...
    async def get_users_for_mass_message(self, mass_message: MassMessage) -> List[User]:
        ...
        async with async_session_maker() as session:
            ...
            if mass_message.is_active_only:
                ...
                filters.append(or_(User.updated_at >= active_date, User.last_free_check >= active_date))
        ...
        async with async_session_maker() as session:
            ...
            for user in users:
                recipient = MassMessageRecipient(
                    mass_message_id=mass_message_id, user_id=user.tg_id, status=OutboxStatus.PENDING
                )
    ...
    async def send_mass_message(self, mass_message_id: int, batch_size: int = 50) -> Dict[str, int]:
        ...
        async with async_session_maker() as session:
            ...
            for i in range(0, total_recipients, batch_size):
                ...
                for recipient, result in zip(batch, results):
                    if isinstance(result, Exception):
                        ...
                        failed_count += 1
                        ErrorHandler.log_error(result, f"отправка пользователю {recipient.user_id}", recipient.user_id)
                    else:
            ...
            return {"success": success_count, "failed": failed_count, "total": total_recipients}

    @get_retry_decorator("telegram")
    ...
    async def _send_to_recipient(self, mass_message: MassMessage, recipient: MassMessageRecipient) -> bool:
        ...
        try:
            ...
            await self.bot.send_message(chat_id=recipient.user_id, text=message_text, parse_mode="Markdown")

            return True

        except Exception as e:
            ErrorHandler.log_error(e, f"отправка пользователю {recipient.user_id}", recipient.user_id)
            raise e

    @get_retry_decorator("db")
    async def schedule_mass_message(self, mass_message_id: int, scheduled_at: datetime) -> bool:
    ...
    async def get_mass_message_stats(self, mass_message_id: int) -> Dict[str, Any]:
        ...
        async with async_session_maker() as session:
            ...
            query = (
                select(MassMessageRecipient.status, func.count(MassMessageRecipient.id))
                .where(MassMessageRecipient.mass_message_id == mass_message_id)
    ...
    async def get_scheduled_messages(self) -> List[MassMessage]:
        ...
        async with async_session_maker() as session:
            query = select(MassMessage).where(
                and_(MassMessage.status == MassMessageStatus.SCHEDULED, MassMessage.scheduled_at <= datetime.utcnow())
            )
            result = await session.execute(query)
    ...
    async def process_scheduled_messages(self) -> Dict[str, int]:
        ...
        for message in scheduled_messages:
            try:
                ...
                ErrorHandler.log_error(e, f"обработка запланированного сообщения {message.id}")
                total_failed += 1

        logger.info(
            f"Обработано запланированных сообщений: {total_processed}, успешно: {total_success}, неудачно: {total_failed}"
        )
