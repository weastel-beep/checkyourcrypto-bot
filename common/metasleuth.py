...
class MetaSleuthAPI:
    ...
    async def _get_session(self, api_type: str = "screening") -> aiohttp.ClientSession:
        ...
        if self.session is None or self.session.closed:
            ...
            self.session = aiohttp.ClientSession(
                headers={"API-KEY": api_key, "Content-Type": "application/json"}, timeout=aiohttp.ClientTimeout(total=30)
            )
        else:
            # Обновляем заголовки для существующей сессии
            api_key = self.wallet_screening_key if api_type == "screening" else self.address_label_key
            self.session._default_headers.update({"API-KEY": api_key})
        return self.session
    ...
    async def wallet_screening(self, address: str, chain: str) -> Dict:
        ...
        """
        logger.info(f"🔍 MetaSleuth.wallet_screening ВЫЗВАН для адреса {address} и сети {chain}")
        logger.info(f"🔍 settings.mock_metasleuth = {settings.mock_metasleuth}")
        
        if settings.mock_metasleuth:
            logger.info("🔍 ИСПОЛЬЗУЕМ MOCK данные")
        ...
        try:
            logger.info("🔍 MetaSleuth: ПОЛУЧАЕМ сессию")
            session = await self._get_session("screening")
            logger.info("🔍 MetaSleuth: СЕССИЯ получена")
            
            url = f"{self.base_url}/address-compliance/api/v3/risk-score"
            logger.info(f"🔍 MetaSleuth: URL = {url}")

            # Конвертируем chain в chain_id согласно документации
            chain_id_map = {"eth": 1, "btc": -1, "tron": -2, "solana": -3}
            chain_id = chain_id_map.get(chain.lower(), 1)  # По умолчанию ETH

            payload = {"address": address, "chain_id": chain_id, "interaction_risk": True}

            logger.info(f"🔍 MetaSleuth: ОТПРАВЛЯЕМ POST запрос с payload = {payload}")
            async with session.post(url, json=payload) as response:
                logger.info(f"🔍 MetaSleuth: ПОЛУЧЕН ответ со статусом {response.status}")
                ...
                if data.get("code") == 200000 and "data" in data:
                    ...
                    result = {"risk": 0, "details": ["Ошибка обработки ответа API"], "percents": {}, "risk_indicators": []}
    ...
    async def _mock_wallet_screening(self, address: str, chain: str) -> Dict:
        ...
        return {
            "risk": 2,
            "details": ["Адрес не связан с известными мошенническими схемами", "Низкая активность транзакций"],
            "percents": {"scam": 5, "mixer": 0, "exchange": 15, "gambling": 0},
        }

    async def _mock_address_label(self, address: str, chain: str) -> Dict:
        """Мок данные для address label"""
        return {"label": "Обычный пользователь", "website": "", "type": "user", "tags": ["active", "regular"]}
