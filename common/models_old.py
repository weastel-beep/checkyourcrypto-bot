...
class Text(Base):
    """Модель текстов (мультиязычность) - старая таблица"""
...
class BotText(Base):
    """Модель текстов бота (основная таблица Bot Flow Designer)"""
...
class Scenario(Base):
    ...
    stages: Mapped[dict] = mapped_column(JSON, nullable=True)  # JSON с этапами сценария
