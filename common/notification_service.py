...
class NotificationService:
    ...
    async def send_notification(
        ...
        try:
            ...
            await self.bot.send_message(chat_id=user_id, text=message, reply_markup=keyboard, parse_mode="Markdown")

            # Сохраняем уведомление в базу
            async with async_session_maker() as session:
                notification = Notification(
                    user_id=user_id, message=message, notification_type=notification_type, sent_at=datetime.utcnow()
                )
            ...
            ErrorHandler.log_error(e, f"отправка уведомления пользователю {user_id}", user_id)
            return False

    async def send_bulk_notification(
        self, user_ids: List[int], message: str, notification_type: NotificationType = NotificationType.INFO
    ) -> Dict[str, int]:
...
Используйте кнопку "Пополнить" в главном меню."""
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="💰 Пополнить", callback_data="topup")]])
    ...
    async def send_new_feature_announcement(self, feature_name: str, description: str) -> Dict[str, int]:
        ...
        async with async_session_maker() as session:
            ...
            users = await session.execute("SELECT tg_id FROM users WHERE is_active = true")
